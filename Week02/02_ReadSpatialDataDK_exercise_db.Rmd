...
# playgrounds: 51 features, points
# shelters: 19 features, points
...
```{r}
# forests: ETRS89 / UTM zone 32N
# parks: ETRS89 / UTM zone 32N
# playgrounds: NA
# shelters: NA
```
...
over5ha <- parks %>% filter(areal_m2 > 50000)
...
big_forests <- forests %>% filter(unclass(areas) > 300000)
...
```{r}
dim(aarhus) # res: 3334 x 3334, 5 layers
```
...
crs(elevation) # EPSG: 9122
crs(aarhus) #EPSG: 9001
...
# Do you see the playgrounds? NO
plotRGB(aarhus, stretch = "lin");plot(playgrounds, add = TRUE)

# We need to use the playgrounds that we made in the previous exercise because
# it has the correct crs
plotRGB(aarhus, stretch = "lin");plot(playgrounds_crs, add = TRUE)
...
extent(municipalities)
dim(municipalities)
plot(municipalities)

# transform crs - doesn't work??
municipalities_crs <- projectRaster(municipalities, crs = the_crs, method = "ngb")
```
