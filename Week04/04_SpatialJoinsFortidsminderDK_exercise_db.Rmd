...
municipalities_25832 <- municipalities %>% 
  st_as_sf(municipalities) %>% st_transform(crs = 25832)
...
plot(municipalities_25832[st_intersection(least, municipalities_25832, sparse = FALSE), ]$geometry, col = "red", add = T) 
...
nb <- data.frame(place = "Nobel", longitude=10.2033485, latitude=56.174346)
nb_buf <- nb %>% 
...
st_crs(viking_clip) == st_crs(nobel) # TRUE
...
medieval <- monuments %>% dplyr::filter(datering == "Middelalder")

crs(medieval) == crs(municipalities_25832)

medi_muni <- st_join(medieval, municipalities_25832)
# /End Code/ #
```
